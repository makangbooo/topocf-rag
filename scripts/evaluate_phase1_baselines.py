#!/usr/bin/env python3
"""Evaluate text-free Phase 1 BM25 and bge-m3 baseline score caches."""
...
def fingerprint_model(root: Path) -> str:
    """Hash model configuration and every local dense/sparse weight artifact."""
...
def parse_args() -> argparse.Namespace:
    ...
    # Exact-token audit: frozen train/dev serializations peak at 622/772 bge
    # tokens respectively.  1024 preserves every input while retaining the
    # hard failure if a future manifest crosses the configured limit.
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("reports/phase1/baselines.json")
    )
    parser.add_argument("--force-recompute", action="store_true")
    parser.add_argument(
        "--allow-truncation",
        action="store_true",
        help="Allow candidate truncation; default behavior is to fail.",
    )
...
def evaluate_one_split(
    *,
    ...
    cache_path = cache_dir / f"{split_key}_baseline_scores.json"
    scores = None if force_recompute else load_score_cache(
        cache_path, prepared, config
    )
    if scores is None:
        scores = score_prepared_split(
            prepared, embedder, bm25_config=bm25_config
        )
        atomic_json_dump(build_score_cache(prepared, scores, config), cache_path)
...
def main() -> int:
    ...
    common = {
        ...
        "bm25_config": bm25_config,
        "max_length": args.max_length,
        "fail_on_truncation": not args.allow_truncation,
